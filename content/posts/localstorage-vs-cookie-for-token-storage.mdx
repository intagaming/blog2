...
> **DISCLAIMER**: This is simplified. In practice, the request is more likely to
> be in the form of a `POST` request instead of `GET` like the below CSRF example.
...
attempt is flawed.][2] Then, [read this from StackOverflow if you store the
csrfToken not in the cookie, but in localStorage][3] (obviously not). If you go
as far as using `httpOnly SameSite=Strict Secure` settings, remember that
...
Have you realized yet? You've just achieved Cross-Site Scripting. If your
